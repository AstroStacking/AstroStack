...
double Powell::operator()(const Eigen::VectorXd& x)
{
    return std::pow(x(0) + 10 * x(1), 2) + 5 * std::pow(x(2) - x(3), 2) + std::pow(x(1) - 2 * x(2), 4) +
           10 * std::pow(x(0) - x(3), 4);
}

Eigen::VectorXd Powell::gradient(const Eigen::VectorXd& x)
{
    Eigen::VectorXd grad(x.size());
    grad << 2 * (x(0) + 10 * x(1)) + 40 * std::pow(x(0) - x(3), 3),
            4 * std::pow(x(1) - 2 * x(2), 3) + 20 * (x(0) + 10 * x(1)),
            10 * (x(2) - x(3)) - 8 * std::pow(x(1) - 2 * x(2), 3),
            -10 * (x(2) - x(3)) - 40 * std::pow(x(0) - x(3), 3);
    return grad;
}

Eigen::MatrixXd Powell::hessian(const Eigen::VectorXd& x)
{
    Eigen::MatrixXd mat(x.size(), x.size());

    mat << 2 + 120 * std::pow(x(0) - x(3), 2), 20, 0, -120 * std::pow(x(0) - x(3), 2),
        20, 12 * std::pow(x(1) - 2 * x(2), 2) + 200, -24 * std::pow(x(1) - 2 * x(2), 2), 0,
        0, -24 * std::pow(x(1) - 2 * x(2), 2), 10 + 48 * std::pow(x(1) - 2 * x(2), 2), -10,
